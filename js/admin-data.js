...
async function setupCompetitionPeriods(seasonId) {

    console.log("admin-data.js: setupCompetitionPeriods Called", seasonId);

    // ==========================================
    // CHECK EXISTING PERIODS
    // ==========================================

    const {data: existingPeriods, error: existingError} =
        await supabaseClient
    ...
    if (existingPeriods && existingPeriods.length > 0) {
        throw new Error(`This season already has ${existingPeriods.length} competition period(s).`);
    }
    ...
    const records = periods.map(period => ({season_id: seasonId,...period}));
...
async function getSeasonById(seasonId) {
    ...
    console.log("admin-data.js: getSeasonById Called");
...
async function getActivePlayerCount(seasonId) {

    console.log("admin-data.js: getActivePlayerCount Called");
...
async function getCompetitionPeriods(seasonId) {
    ...
    console.log("admin-data.js: getCompetitionPeriods Called");

    const {data, error} =
        await supabaseClient
            .from("competition_periods")
            .select(`period_number, start_gameweek, end_gameweek`)
            .eq("season_id", seasonId)
            .order("period_number");
    if (error)
        throw error;
...
async function checkCompetitionPeriods(seasonId) {

    console.log("admin-data.js: checkCompetitionPeriods Called");
...
async function loadPrizeSettings(seasonId) {

    console.log("admin-data.js: loadPrizeSettings Called");
...
async function savePrizeSettings(seasonId, settings) {

    console.log("admin-data.js: savePrizeSettings Called");

    const record = {
        season_id: seasonId,
        entry_fee: settings.entryFee,
        overall_percent: settings.overallPercent,
        period_percent: settings.periodPercent,
        highest_gameweek_percent: settings.highestGameweekPercent,
        captain_percent: settings.captainPercent,
    ...
    const {data, error} =
        await supabaseClient
            .from("season_prize_settings")
            .upsert(record, {onConflict: "season_id"})
            .select()
            .single();
    if (error)
        throw error;
...
async function activateSeason(seasonId) {
    ...
    const {error} =
        await supabaseClient
            .rpc("activate_season", {target_season_id:seasonId});
    if (error)
        throw error;
}
...
async function createSeason(seasonCode, seasonName, totalGameweeks, currentGameweek) {
    ...
    console.log("admin-data.js: createSeason Called");
...
async function copySeasonPlayers(targetSeasonId) {

    console.log("admin-data.js: copySeasonPlayers Called");

    // ==========================================
    // GET ACTIVE SEASON
    // ==========================================

    const activeSeason =  await getAdminActiveSeason();

    if (activeSeason.id === targetSeasonId) {
        throw new Error("Cannot copy players to the active season.");
    }
    ...
    const sourcePlayers = await getSeasonPlayers(activeSeason.id);

    if (!sourcePlayers ||  sourcePlayers.length === 0) {
        throw new Error("The active season has no players to copy.");
    }
    ...
    const targetPlayers = await getAllSeasonPlayers(targetSeasonId);

    if (targetPlayers.length > 0) {
        throw new Error("The target season already contains players.");
    }
    ...
    const records = sourcePlayers.map(player => ({
                season_id: targetSeasonId,
                player_id: player.player_id,
                fpl_entry_id: player.fpl_entry_id,
                fpl_team_name: player.fpl_team_name,
                active: true,
                display_order: player.display_order ?? 999
            })
        );
